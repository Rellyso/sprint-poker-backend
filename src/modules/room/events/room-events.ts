...
export function setupRoomEvents(io: Server, socket: Socket, roomService: RoomService) {
  ...
  socket.on('/room/player/vote', async ({ roomId, vote }: { roomId: string, vote: string }) => {
    ...
    try {
      ...
      const { session, players } = await roomService.submitVote(roomId, userId, vote);
  ...
  // Evento para revelar votos
  socket.on('/room/game-type/update', async (roomId: string, game_type: GameType) => {
    if (game_type !== GameType.fibonacci && game_type !== GameType.decimal) {
      return;
    }

    try {
      const session = await roomService.updateSessionGameType(roomId, game_type);
  ...
  socket.on('/room/join', async ({ roomId, userId }) => {
    try {
      const { session, playersInRoom } = await roomService.addPlayerToSession(roomId, userId);
